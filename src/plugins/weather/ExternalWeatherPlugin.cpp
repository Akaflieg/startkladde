...
void ExternalWeatherPlugin::refresh ()
{
    ...
    QString resolvedWithArgs;
    if (parameters.isEmpty()) {
        resolvedWithArgs = resolved;
    } else {
        resolvedWithArgs = resolved + notr(" ") + parameters;
    }

    if (!process->startAndWait (resolvedWithArgs)) OUTPUT_AND_RETURN (tr ("Error: %1").arg (process->getProcess ()->errorString ()));
